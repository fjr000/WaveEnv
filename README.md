...
- 📍 **单点查询**：基于区域模拟结果，通过空间双线性插值查询任意点的海浪高度（无时间插值，使用最近帧）
- ⚙️ **参数配置**：支持风场、波浪谱、离散化等参数的前端配置与实时调整
- 🎨 **可视化展示**：动态渲染海浪高度场，支持时间序列播放与交互查询（可禁用以提升性能）
- ⏯️ **实时控制**：支持暂停、恢复、停止模拟任务，一次点击立即生效
- 👥 **多用户支持**：Session 隔离，每个用户独立管理自己的仿真任务，互不干扰
...
如果遇到 `container waveenv-backend is unhealthy` 错误：

1. **查看容器日志**：
   ```bash
   docker logs waveenv-backend
   ```
...
- **HTTP 连接复用**：前端使用 Session 级别的连接池，查询响应时间从 2.7秒降至 14ms（性能提升 99.5%）
- **后端缓存优化**：单帧网格缓存机制，避免重复构建，同帧多次查询耗时降至毫秒级
- **可选的图表渲染**：支持禁用实时等高线图，大幅提升界面响应速度，特别适合纯查询场景

### 用户体验

- **即时响应**：控制按钮（暂停/恢复/停止）一次点击立即生效，无需等待
- **快速查询**：单点查询功能毫秒级响应，不受显示刷新频率影响
- **时间同步**：前端显示的模拟时间始终与后端最新帧时间一致

### 多用户支持

- **Session 隔离**：每个浏览器标签页（Session）独立管理自己的仿真任务
- **资源隔离**：不同用户的任务互不干扰，支持并发使用
...
- ✅ Docker 容器化部署支持
- ✅ 单元测试和集成测试完成
- ✅ 性能优化完成（连接复用、缓存机制、可选图表渲染）
- ✅ 用户体验优化完成（即时响应、快速查询、时间同步）
- ✅ 多用户支持完成（Session 隔离、资源隔离）
